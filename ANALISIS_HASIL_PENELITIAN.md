...
| **Test set**             | _Identik di seluruh 12 eksperimen_ (`random_state=42`, stratifikasi `label`) — 26 citra: aneurysm 9, tumor 8, cancer 9 |
...
> **Catatan:** Seluruh backbone diinisialisasi dengan bobot pretrained ImageNet (transfer learning). Normalisasi yang digunakan adalah statistik ImageNet (`mean=[0.485, 0.456, 0.406]`, `std=[0.229, 0.224, 0.225]`) sebagaimana standar fine-tuning model torchvision. Dataset yang digunakan adalah citra CT-Brain — **bukan berasal dari ImageNet**.
...
- **LR 1e-2 ⇒ degradasi masif** terutama pada VGG16 yang collapse menjadi _majority-class classifier_ (selalu prediksi "cancer" ⇒ 9/26 = 34.6% = accuracy baseline).
...
**Insight praktis:** Untuk fine-tuning medical imaging data kecil (<300 citra), ResNet50 memberikan _best trade-off_ antara performa, robustness terhadap hyperparameter, dan stabilitas training.
...
- **ViT**: turun dari 0.9615 ke 0.9231 saat training sampel berkurang dari 204 ke 178 (−26 sampel). Konfirmasi hipotesis _data-hungry nature_ ViT.
...
- **Cancer → Aneurysm** adalah _satu-satunya_ misklasifikasi yang terjadi (1 dari 9 sampel cancer). Tidak ada aneurysm → cancer atau tumor ↔ lainnya pada LR=1e-4.
...
Pada LR=1e-2, pola kesalahan berpola ke _mode collapse_:
...
1. **Learning rate sensitivity analysis** pada fine-tuning medical image classification tiga arsitektur — menunjukkan bahwa LR 1e-2 membuat model SOTA _underperform_ majority baseline (VGG16 acc 0.346 ≈ 34.6% = proporsi kelas mayoritas).
...
- [ ] Tulis diskusi tentang _why LR matters more than architecture in small-data fine-tuning_
...
_Dibuat: 13 Juni 2026 | Berdasarkan 12 notebook training di `skenario_1/` dan `skenario_2/`_
---
